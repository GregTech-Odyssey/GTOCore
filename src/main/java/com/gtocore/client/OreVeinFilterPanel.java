...
public final class OreVeinFilterPanel {
    ...
    // Top-left of the panel lives in OreVeinFilter so it survives a restart; unset until first shown,
    // then right-middle, and the user can drag it anywhere from there.
    // Gap kept between the panel and the right edge of the screen at the default position.
    ...
    public static boolean mouseReleased() {
        if (dragging) {
            dragging = false;
            OreVeinFilter.save(); // remember where the drag left the panel
            return true;
        }
        return false;
    }
    ...
    // Right-middle on first use, then keep whatever the drag left, always clamped inside the screen
    // (a saved position from a bigger window must not push the panel off a smaller one).
    private static void ensurePos(int screenW, int screenH) {
        int ph = panelHeight(screenH);
        int x = OreVeinFilter.getPanelX();
        int y = OreVeinFilter.getPanelY();
        if (x == OreVeinFilter.UNSET || y == OreVeinFilter.UNSET) {
            x = screenW - PANEL_WIDTH - EDGE_MARGIN;
            y = (screenH - ph) / 2;
        }
        x = Math.max(0, Math.min(x, screenW - PANEL_WIDTH));
        y = Math.max(0, Math.min(y, screenH - ph));
        OreVeinFilter.setPanelPos(x, y);
    }

    private static int panelX(int screenW) {
        return OreVeinFilter.getPanelX();
    }

    private static int panelY(int screenH) {
        return OreVeinFilter.getPanelY();
    }
    ...
    public static void render(GuiGraphics g, Font font, int screenW, int screenH, int mouseX, int mouseY, ResourceKey<Level> dim) {
        ensurePos(screenW, screenH);
        if (dragging) {
            OreVeinFilter.setPanelPos(mouseX - dragGrabX, mouseY - dragGrabY);
            ensurePos(screenW, screenH);
        }
