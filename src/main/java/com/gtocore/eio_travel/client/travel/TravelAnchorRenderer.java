...
public interface TravelAnchorRenderer<T extends ITravelTarget> extends TravelRenderer<T> {

    RenderType BOLD_LINES = OutlineRenderType.createLines("bold_lines", 3);
    RenderType VERY_BOLD_LINES = OutlineRenderType.createLines("very_bold_lines", 5);

    @Override
    default void render(T travelData, LevelRenderer levelRenderer, PoseStack poseStack, double distanceSquared, boolean active) {
        var renderInfo = getRenderInfo(travelData);
        if (!renderInfo.visibility()) {
            return;
        }
        ...
        // Render Text
        if (!renderInfo.name().trim().isEmpty()) {
            // Scale for rendering
            ...
            Component tc = Component.literal(renderInfo.name().trim());
        ...
        if (renderInfo.icon() != Items.AIR) {
            ...
            poseStack.scale(-scale, scale, -scale);
            ItemStack stack = new ItemStack(renderInfo.icon());
    ...
    record RenderInfo(boolean visibility, String name, Item icon) {}

    RenderInfo getRenderInfo(T travelData);
}
