 ...
 * <a href="https://github.com/castcrafter/travel_anchors">https://github.com/castcrafter/travel_anchors</a> for
 * allowing us to use their code with our license.
...
public class TravelHandler {
    ...
    private static boolean canItemTeleport(Player player, InteractionHand hand) {
        ...
        return stack.is(GTOItems.TRAVEL_STAFF.asItem());
    }

    public static boolean canBlockTeleport(Player player) {
        return (player.level().getBlockState(player.blockPosition().immutable().below()).getBlock() == GTOMachines.TRAVEL_ANCHOR.get());
    }
    ...
    public static boolean blockTeleport(Level level, Player player, boolean sendToServer) {
        return getTeleportAnchorTarget(player)
                .filter(iTravelTarget -> blockTeleportAround(level, player, iTravelTarget.getPos(), sendToServer)).isPresent();
    }
    ...
    public static boolean blockTeleportAround(Level level, Player player, BlockPos pos, boolean sendToServer) {
        return blockTeleportTo(level, player, pos, sendToServer) ||
                blockTeleportTo(level, player, pos.relative(Direction.NORTH), sendToServer) ||
                blockTeleportTo(level, player, pos.relative(Direction.SOUTH), sendToServer) ||
                blockTeleportTo(level, player, pos.relative(Direction.EAST), sendToServer) ||
                blockTeleportTo(level, player, pos.relative(Direction.WEST), sendToServer);
    }

    private static boolean blockTeleportTo(Level level, Player player, BlockPos pos, boolean sendToServer) {
        Optional<Double> height = player.level().isClientSide() ? Optional.of(2d) : isTeleportPositionClear(level, pos);
        if (height.isEmpty()) {
            return false;
        }
    ...
    public static Optional<Vec3> teleportPosition(Level level, Player player) {
        ...
        int range = GTOConfig.INSTANCE.travelConfig.blinkRange;
    ...
    public static Optional<ITravelTarget> getElevatorAnchorTarget(Player player, Direction direction) {
        int anchorRange = GTOConfig.INSTANCE.travelConfig.blockRange;
