...
public class PlayerHeadUtils {
    ...
    public static ItemStack get_GregoriusT_Head() {
        return createPlayerHeadFromNBT(GregoriusT);
    }
    ...
    public static ItemStack get_maple197_Head() {
        return createPlayerHeadFromNBT(maple197);
    }
     ...
     */
    public static ItemStack createPlayerHeadFromNBT(String nbtString) {
        ItemStack head = new ItemStack(Items.PLAYER_HEAD);
        try {
            // 解析 NBT 字符串为 CompoundTag
            CompoundTag tag = TagParser.parseTag(nbtString);
            // 将解析后的 NBT 标签设置到物品上
            head.setTag(tag);
        } catch (Exception e) {
            GTOCore.LOGGER.error("Head generation error: ", e);
        }
        return head;
    }
}
