...
public record TradeEntry(
    ...
    private int checkInputEnough(TradeData data) {
        ...
        int inputCurrencies = (int) (inputGroup().currencies().isEmpty() ? Integer.MAX_VALUE : inputGroup().currencies().object2LongEntrySet().stream()
                .filter(entry -> entry.getLongValue() != 0)
                .mapToLong(entry -> (WalletUtils.getCurrencyAmount(data.getUuid(), serverLevel, entry.getKey()) / entry.getLongValue())).min().orElse(0));
