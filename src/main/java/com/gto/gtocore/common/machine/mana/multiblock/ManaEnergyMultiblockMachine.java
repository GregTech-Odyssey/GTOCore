...
public class ManaEnergyMultiblockMachine extends ManaMultiblockMachine implements IManaEnergyMachine, IOverclockConfigMachine {
    ...
    protected GTRecipe getRealRecipe(@NotNull GTRecipe recipe) {
        ...
        if (eu > 0) {
            recipe = GTORecipeModifiers.externalEnergyOverclocking(this, recipe, eu, getManaContainer().getMaxIORate(), true, 1, 1);
            return recipe;
        } else {
            return GTORecipeModifiers.manaOverclocking(this, recipe, getManaContainer().getMaxIORate(), true, 1, 1);
        }
    }
